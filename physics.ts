...
export const calculateStability = (metadata: NodeMetadata): number => {
  const { multiplier, lastHandshakeTimestamp, peerCount = 1 } = metadata;
  ...
  // Higher peer density increases mesh entropy, slightly dampening individual stability indices.
  const heatCoefficient = 0.02;
  const H = 1 + (peerCount * heatCoefficient);

  // 4. Compute Integration / Stability Index (I = (Vh * Rn^2) / H)
  const stabilityIndex = (Vh * Math.pow(Rn, 2)) / H;
