  ...
  async getPrisonSectors() {
    ...
    return snapshot.docs.map(sDoc => ({
      ...
      nandFlushSentence: async () => {
        ...
        return updateDoc(doc(db, 'sectors', sDoc.id), { status: 'CLEARED', type: 'HEALING_CENTER' });
      },
      convertToHealingCenter: async () => {
        return updateDoc(doc(db, 'sectors', sDoc.id), { architecture: 'RESONANCE_GROUNDING' });
      }
    }));
  },
