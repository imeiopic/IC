...
const filteredBonds = computed(() => {
    ...
    if (searchQuery.value.trim()) {
        ...
        result = result.filter(bond =>
            bond.payload.description.toLowerCase().includes(query)
        );
    }
...
const getStatusClass = (status: string) => {
    switch (status) {
        case 'LOCKED': return 'bg-success';
        case 'SYNCING': return 'bg-warning text-dark';
        case 'DISRUPTED': return 'bg-danger';
                        ...
                        <form @submit.prevent="handleCreateBond">
                            <div class="mb-3">
                                ...
                                <input v-model="description" type="text"
                                    ...
                                    placeholder="Order specifics..." required>
                            </div>
                            <div class="mb-3" v-if="menu">
                                <label class="form-label small text-secondary text-uppercase d-block">Presenting
                                    Menu</label>
                                <div class="list-group list-group-flush border border-secondary rounded">
                                    <button v-for="item in menu.items" :key="item.id" type="button"
                            ...
                            <button type="submit" class="btn btn-primary w-100 fw-bold mt-2" :disabled="isInitializing">
                ...
                <div class="card shadow-sm border-0">
                    <div
                        ...
                        <div class="btn-group btn-group-sm ms-auto ms-md-0 shadow-none">
                            <button class="btn" :class="sortBy === 'date' ? 'btn-primary' : 'btn-outline-primary'"
                                @click="sortBy = 'date'">
                                Date
                            </button>
                            <button class="btn" :class="sortBy === 'value' ? 'btn-primary' : 'btn-outline-primary'"
                                @click="sortBy = 'value'">
                                Value
                            </button>
                        </div>
                    ...
                    <div class="card-body">
                        ...
                        <div v-else-if="filteredBonds.length === 0" class="text-center py-4 text-muted italic">
                            {{ searchQuery ? 'No matches found for "' + searchQuery + '" in this thread.' : 'No logical
                            bonds detected in the current thread.' }}
                        </div>
                                ...
                                <tbody>
                                    <tr v-for="bond in filteredBonds" :key="bond.id">
                                        <td>
                                            ...
                                            <div class="mt-1">
                                                <span v-for="item in bond.payload.items" :key="item.id"
                                                    class="badge bg-secondary me-1" style="font-size: 0.6rem;">{{
                                                        item.name }}</span>
                                            </div>
                                            <small class="text-primary d-block mt-1">{{ bond.payload.totalValue }} {{
                                                bond.payload.currency }}</small>
                                        </td>
                                        <td style="min-width: 150px;">
                                            ...
                                            <small class="font-monospace text-secondary">{{ bond.verificationThreads
                                                }}/16 Threads</small>
                                        </td>
                                        <td>
                                            <span class="badge" :class="getStatusClass(bond.status)">{{ bond.status
                                                }}</span>
                                        </td>
                                        <td>
                                            <button v-if="bond.status !== 'LOCKED' && bond.status !== 'DISRUPTED'"
