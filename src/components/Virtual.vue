<template>
  <CContainer fluid class="vre-substrate p-0 bg-black min-vh-100 overflow-hidden font-mono text-white">
    
    <div class="vre-hud fixed-top p-4 d-flex justify-content-between align-items-start z-max pointer-events-none">
    ...
    <div class="world-viewport position-relative w-100 vh-100 bg-zinc-950 d-flex align-items-center justify-content-center">
      ...
      <div class="reality-anchor position-relative animate-float">
        <div class="core-ring border border-info rounded-circle position-absolute translate-middle top-50 start-50"></div>
        <div class="core-glow position-absolute translate-middle top-50 start-50"></div>
        <img :src="entityData?.photoURL || '/images/imeiopic.png'" class="rounded-circle border border-info p-1 shadow-glow position-relative grayscale" width="120" />
      </div>
    ...
    <div class="fixed-bottom p-4 z-max d-flex justify-content-center">
      <div class="dock-substrate bg-zinc-900 bg-opacity-80 border border-zinc-800 rounded-pill px-4 py-2 shadow-lg d-flex gap-4 backdrop-blur">
        ...
        <CButton color="link" class="text-success p-0" title="COMMERCE" @click="transit('Commerce')">
...
<style scoped>
.vre-substrate { cursor: crosshair; }
.z-max { z-index: 1000; }
.pointer-events-none { pointer-events: none; }
.pointer-events-auto { pointer-events: auto; }
.backdrop-blur { backdrop-filter: blur(10px); }
.text-glow { text-shadow: 0 0 15px rgba(0, 229, 255, 0.6); }

/* WORLD ANIMATIONS */
.animate-float { animation: float-core 6s ease-in-out infinite; }
...
.shadow-glow { box-shadow: 0 0 40px rgba(0, 229, 255, 0.2); }
