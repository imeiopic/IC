...
const baseSchema = z.object({
  ...
  VITE_FIREBASE_MEASUREMENT_ID: z.string().min(1, 'VITE_FIREBASE_MEASUREMENT_ID is required'),
...
const serverSchema = z.object({
  ...
  GOOGLE_APPLICATION_CREDENTIALS: z.string().min(1, 'Path to Service Account JSON is required'),
});
...
const devSchema = baseSchema
  ...
  .superRefine((data, ctx) => {
    if (data.VITE_SSL_STRATEGY === 'custom' || data.VITE_SSL_STRATEGY === 'devcert') {
      if (!data.VITE_SSL_KEY_PATH) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: "VITE_SSL_KEY_PATH is required when VITE_SSL_STRATEGY is 'custom'",
      ...
      if (!data.VITE_SSL_CERT_PATH) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: "VITE_SSL_CERT_PATH is required when VITE_SSL_STRATEGY is 'custom'",
