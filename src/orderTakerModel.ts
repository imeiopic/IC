import {
  ...
  getDocs,
  type Unsubscribe
} from 'firebase/firestore'; // Standardized to firebase.ts
...
export const OrderTakerModel = {
   ...
   */
  async initializeBond(
    initiatorUid: string,
    targetUid: string,
    data: OrderBond['payload'],
    vreId: string,
    assignedMemberUid?: string,
    targetCoords?: OrderBond['targetCoords']
  ) {
  ...
  subscribeToBonds(userUid: string, onUpdate: (bonds: OrderBond[]) => void): Unsubscribe {
    ...
    const q = query(bondsRef, where('initiatorUid', '==', userUid), orderBy('createdAt', 'desc'));

    return safeOnSnapshot(
      q,
      (snapshot) => {
        const bonds = snapshot.docs.map(
          (doc) =>
            ({
              id: doc.id,
              ...doc.data()
   ...
   */
  subscribeToAssignedBonds(
    userUid: string,
    onSnapshotReceived: (snapshot: any) => void
  ): Unsubscribe {
    const bondsRef = collection(db, 'bonds');
    const q = query(
      bondsRef,
