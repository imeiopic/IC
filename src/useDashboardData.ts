...
export function useDashboardData(options: DashboardOptions = {}) {
    ...
    function setupListeners() {
        ...
        // 1. Profile Listener
        unsubscribeProfile = safeOnSnapshot(doc(db, 'invitees', uid), (snap) => {
        ...
        unsubscribeBonds = OrderTakerModel.subscribeToAssignedBonds(uid, (snap) => {
            ...
            if (options.onNewBond && assignedBonds.value.length > 0 && newBonds.length > assignedBonds.value.length) {
