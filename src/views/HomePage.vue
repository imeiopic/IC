  ...
  >
    ...
    >
      ...
      <div v-else class="authenticated-flow">
        ...
        <p v-if="bankLinkingStore.bankLinkingError" class="text-danger tiny mt-3">
          {{ bankLinkingStore.bankLinkingError }}
        </p>

        <!-- Bank Linking Section -->
        <div class="bank-linking-section mt-5 p-3 border border-zinc-700 rounded-lg">
          <div v-if="bankLinkingStore.isBankLinkingInProgress">
            <!-- Render SelectBank.vue if linking is in progress -->
            <SelectBank />
          </div>
          <div v-else-if="bankLinkingStore.isBankLinked">
            <!-- Display linked banks if already linked -->
            <div class="text-center">
              <div
                class="status-orb mx-auto mb-2"
                :style="{ backgroundColor: bankLinkingStore.tierColor }"
              ></div>
              <h4 class="text-success italic font-black mb-2">IOWB Node Grounded</h4>
              <p class="text-white small mb-2">Ledger: {{ bankLinkingStore.iowbBalance }} IO$</p>
...
import { loadStripe, type Stripe } from '@stripe/stripe-js'; // Sighting the Stripe SDK
...
import WelcomeShield from '@components/WelcomeShield.vue'; // Import the WelcomeShield component
...
let toastUpdateInterval: any; // Use 'any' for cross-environment compatibility (Node vs. Browser)
...
const nodeSecurityLevel = computed(() => {
  ...
  if (user.value.uid === (import.meta as any).env.VITE_IME_ROOT_SIGNATURE) return 'ROOT';
...
const handleStripeCheckout = async (tier: any) => {
  ...
  try {
    ...
    const stripe = await loadStripe((import.meta as any).env.VITE_STRIPE_PUBLIC_KEY);
    ...
    if (err instanceof ApiError) {
      console.error('STRIPE_INIT_FAILED (ApiError):', err.message, err.status, err.data);
      errorMsg.value = `LOGIC_FRACTURE: ${err.message} (Code: ${err.status})`;
    } else {
...
const verifyTransit = async () => {
  ...
  const sessionId = urlParams.get('session_id'); // Stripe passes this back
  ...
  if (sessionId && user.value) {
    ...
    try {
      ...
      if (err instanceof ApiError) {
        ...
        errorMsg.value = `ERROR: ${err.message} (Code: ${err.status})`;
      } else {
...
// Existing functions from IoHome.vue
const isBankLinkingInProgressComputed = computed(() => isBankLinkingInProgress.value); // Renamed to avoid conflict
...
const fetchUserEntities = async () => {
  ...
  try {
    ...
    if (error instanceof ApiError) {
      ...
      entitiesError.value = `Failed to load entities: ${error.message} (Code: ${error.status})`;
    } else {
...
const initiateBankLinking = () => {
  bankLinkingStore.setBankLinkingError(null); // Corrected: direct call to action
  bankLinkingStore.isBankLinkingInProgress = true; // Corrected: direct access to state
...
const getRemainingTimeStr = () => {
  ...
  const remaining = checkoutStore.EXPIRATION_MS - (Date.now() - checkoutStore.selectedTimestamp);
